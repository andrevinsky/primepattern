...
(function ($) {
  ...
  function fillCbDependencies(view, model) {
    ...
    if (showExtraFillOptions) {
      view.outline.removeProp('disabled').removeAttr('disabled');
    } else {
      view.outline.prop('disabled', 'disabled').attr('disabled', 'disabled');
    }
  }
  ...
  var taskPrepModel = new CrunchTask(function (init, body, fin) {
      var result,
        max,
        current,
        xMax,
        yMax;
      init(function (_xMax, _yMax) {
        xMax = _xMax;
        yMax = _yMax;
        max = lcm(xMax, yMax);
        current = 0;
      ...
      body(function (resolve, reject, notify) {
        ...
        var moduloCoords, wholePartCoords, isEvenPair, reflectedCoords;
        ...
        wholePartCoords = {
          x: Math.floor(current / xMax),
          y: Math.floor(current / yMax)
        };
    ...
    taskFillAsync = new CrunchTask(function (init, body, fin) {
      ...
      init(function (_patternModel, _maxX, _maxY, _colors, _outlineOnly, _drawing) {
        currentX = 0;
        currentY = 0;
        currentRowColor = 0;
        currentColor = 0;
        ...
        maxY = _maxY;
        colorBank = _colors;
        mode = _outlineOnly ? 'outline' : 'full';
        drawing = _drawing;
        result = {};
      });
  ...
  function prepareModel(thread, xMax, yMax, progressTrack) {
    return new $.Deferred(function (def) {
      try {
        var runInst = taskPrepModel.run(xMax, yMax).done(def.resolve).fail(def.reject).progress(progressTrack);
        thread.fail(runInst.abort);
      } catch (ex) {
        def.reject(ex)
      }
    }).promise();
  ...
  function drawPatterns(view, model) {
    ...
    model.thread = $.Deferred(function (def) {
      ...
      var modelPreparation = prepareModel(def, xMax, yMax, progressBar.handle);

      modelPreparation.always(progressBar.destroy);
  ...
  function getProgressBar(){
    ...
    var bar = $(barTemplate).appendTo('body');
    var gauge = bar.find('.b_progress-gauge');
    var lastProgress = 0;
  ...
  function fillAsync(thread, options) {

    return new $.Deferred(function (def) {
      try {
        var runInst = taskFillAsync.run(
          options.model,
          options.maxX,
          options.maxY,
          options.colorBank,
          options.mode,
          options.drawing).done(def.resolve).fail(def.reject);
